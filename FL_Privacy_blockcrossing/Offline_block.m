%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Function: Off-cond RIS-FL vs normal FL (T). %%%
%%% Author: TSF                                 %%%
%%% Time: 2021.05.13                            %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
...
epsilon = 30;     % DP guidelines (it will change)
delta = 0.01;
SNR_d = 35;
...
noise_noma = normrnd(0,0.5*N0,100,d);
...
for T_noma = 0:5:100
    ...
    zeta_tem = zeros(1,T_noma);
    zeta_sq = 0;
    F_noma(T_noma/5+1) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma(T_noma/5+1) = (F_noma(T_noma/5+1)-F_opt)/F_opt;
end
semilogy(0:5:100,gap_noma,'-r','LineWidth',2,'Marker','o')
hold on
...
for T_noma = 0:5:100
    ...
    theta_opt(:,:,:,T_noma/5+1) = theta_tem;
    ...
    zeta = 0;
    zeta_sq = 0;
    F_noma_cIRS(T_noma/5+1) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma_cIRS(T_noma/5+1) = (F_noma_cIRS(T_noma/5+1)-F_opt)/F_opt;
end
semilogy(0:5:100,gap_noma_cIRS,'-m','LineWidth',2,'Marker','p')
hold on
...
grad = zeros(K,d);
F_noma_IRS = zeros(1,21);
gap_noma_IRS = zeros(1,21);
...
for T_noma = 0:5:100
    ...
    theta_tem = theta_opt(:,:,:,T_noma/5+1);
    ...
    zeta = 0;
    zeta_sq = 0;
    F_noma_IRS(T_noma/5+1) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma_IRS(T_noma/5+1) = (F_noma_IRS(T_noma/5+1)-F_opt)/F_opt;
end
semilogy(0:5:100,gap_noma_IRS,'-m','LineWidth',2,'Marker','p')
hold on
