%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Function: Off-cond RIS-FL vs normal FL (SNR). %%%
%%% Author: TSF                                   %%%
%%% Time: 2021.05.13                              %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
...
%% Note
% This model is for offline condition with different SNR.
...
epsilon = 15;     % DP guidelines (it will change)
delta = 0.01;
% SNR_d = 35;
...
G = aaa.G;%sqrt(kappa_IB/(1+kappa_IB))+sqrt(1/(1+kappa_IB))*r_mat_IB;
hr = aaa.hr;%sqrt(kappa_DI/(1+kappa_DI))+sqrt(1/(1+kappa_DI))*r_mat_DI;
hd = aaa.hd;%sqrt(kappa_DB/(1+kappa_DB))+sqrt(1/(1+kappa_DB))*r_mat_DB;
Random_max = 10000;
...
noise_noma = aaa.noise_noma;%normrnd(0,0.5*N0,T_noma,d);
...
for SNR_d = 5:5:50
    ...
    zeta_tem = zeros(1,T_noma);
    zeta_sq = 0;
    F_noma(SNR_d/5) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma(SNR_d/5) = (F_noma(SNR_d/5)-F_opt)/F_opt;
end
semilogy(5:5:50,gap_noma,'r-','LineWidth',2,'Marker','o')
hold on
...
zeta_tem = zeros(1,T_noma);
grad = zeros(K,d);
F_noma_cIRS = zeros(1,10);
gap_noma_cIRS = zeros(1,10);
g_mat_cIRS = zeros(1,K,eblock);
Rdp = power(sqrt(epsilon+power(double(tmp),2))-double(tmp),2);
threshold = 1e-8;
theta_opt = zeros(N,N,eblock,10);
for SNR_d = 5:5:50
    w = w_ori';
    SNR = 10^(SNR_d/10);
    P = SNR*d/eblock;
    ...
    while temp_value(1)-temp_value(2) > threshold
    ...
    zeta = 0;
    zeta_sq = 0;
    F_noma_cIRS(SNR_d/5) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma_cIRS(SNR_d/5) = (F_noma_cIRS(SNR_d/5)-F_opt)/F_opt;
end
semilogy(5:5:50,gap_noma_cIRS,'-m','LineWidth',2,'Marker','p')
hold on
...
grad = zeros(K,d);
F_noma_IRS = zeros(1,10);
gap_noma_IRS = zeros(1,10);
...
for SNR_d = 5:5:50
    ...
    F_noma_IRS(SNR_d/5) = (1/Dtot)*0.5*(w*U-V)*(w*U-V)' + lamda*w*(w');
    gap_noma_IRS(SNR_d/5) = (F_noma_IRS(SNR_d/5)-F_opt)/F_opt;
end
semilogy(5:5:50,gap_noma_IRS,'-b','LineWidth',2,'Marker','s')
hold on
